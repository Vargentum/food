...
$(function(){
    $('.b-counter').each(function(){
        var valueElement = $(this).find('.counter__value');
        function incrementValue(e){
            valueElement.text(Math.max(parseInt(valueElement.text()) + e.data.increment, 0));
            return false;
        }
        $(this).find('.counter__next').bind('click', {increment: 1}, incrementValue);
        $(this).find('.counter__prev').bind('click', {increment: -1}, incrementValue);
    });
